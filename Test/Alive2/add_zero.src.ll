...
; RUN: llrwt --rules addi-zero-to-x %s -o %t.ll
...
define i32 @test_i32_nsw_nuw(i32 %x) {
  %r = add nsw nuw i32 %x, 0
  ret i32 %r
}

define i64 @test_i64(i64 %x) {
  %r = add nsw nuw i64 %x, 0
  ret i64 %r
}

define i128 @test_i128(i128 %x) {
  %r = add nsw nuw i128 %x, 0
  ret i128 %r
}

define i8 @test_i8(i8 %x) {
  %r = add nsw nuw i8 %x, 0
  ret i8 %r
}

define i7 @test_i7(i7 %x) {
  %r = add nsw nuw i7 %x, 0
  ret i7 %r
}

define i3 @test_i3(i3 %x) {
  %r = add nsw nuw i3 %x, 0
  ret i3 %r
}
